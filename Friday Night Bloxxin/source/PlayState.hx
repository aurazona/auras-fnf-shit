...
class PlayState extends MusicBeatState
{
	...
	var santa:FlxSprite;
	//crossing my road darkness filter
	...
	override public function create()
	{
		...
		switch(SONG.song.toLowerCase())
		{
			...
			case 'bopeebo' | 'crossing-my-road':
			{
				curStage = 'crossroads';
				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('CROSSROADS'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.9, 0.9);
				bg.active = false;
				add(bg);
				
				var ramparts:FlxSprite = new FlxSprite(-650, -600).loadGraphic(Paths.image('BRICK'));
				ramparts.antialiasing = true;
				ramparts.scrollFactor.set(1, 1);
				ramparts.active = false;
				add(ramparts);
		...
		add(dad);
		add(boyfriend);
		if (curStage == 'crossroads') //DARKNESS FUCKERY
		{
			add(darkness);
		}
	...
	*/
	...
	override function beatHit()
	{
		...
		switch (curStage)
		{
			...
			case 'crossroads':
				if (curSong.toLowerCase() == 'crossing-my-road' && curStep == 0)
				{
					/*var darkness:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('dark'));
					darkness.alpha = 0.85;
					darkness.antialiasing = true;
					darkness.scrollFactor.set(1, 1);
					darkness.active = false;
				...
				if (curSong.toLowerCase() == 'crossing-my-road' && curStep == 64)
				{
					darkness.fadeOut(1); //crash on this, null reference??
				}
		}
